...
function saveAs(uri, name) {
  var mimeType = arguments.length <= 2 || arguments[2] === undefined ? "image/png" : arguments[2];
...
var SvgSaver = (function () {
  ...
  _createClass(SvgSaver, [{
    ...
    key: 'as',
    value: function as(el, filename, mimeType, ext) {
      if (isIE11) {
        console.error("svgsaver: asPng not supported on IE11");
      }
      el = SvgSaver.getSvg(el);
      filename = SvgSaver.getFilename(el, filename, ext);
      return saveAs(this.getUri(el), filename, mimeType);
    }
    ...
    value: function asPng(el, filename) {
      return this.as(el, filename, "image/png", "png");
    }
  }, {
    key: 'asJpg',
    value: function asJpg(el, filename) {
      return this.as(el, filename, "image/jpg", "jpg");
    }
  }]);
